...
parser.add_argument(
    "--data_csv",
    type=str,
    default="~/Screen_dataset/dataset/DEKOIS2.csv",
...
parser.add_argument(
    "--ckpt", type=str, default=None, help="Checkpoint to use inside the folder"
)
parser.add_argument(
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Whether to save docking result",
)
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Whether to put ligand on pocket center",
)
parser.add_argument(
    "--keep_input_pose",
    action="store_false",
    default=False,
    help="Whether keep original input pose",
)
parser.add_argument(
    "--use_noise_to_rank",
    action="store_true",
...
parser.add_argument(
    "--run_name",
    type=str,
    default="test_ns_48_nv_10_layer_62023-06-25_07-54-08_model",
    help="",
)
...
parser.add_argument(
    "--surface_path",
    type=str,
    default="~/PDBBind_processed_8A_surface/",
    help="test dataset surface path",
)
parser.add_argument(
    "--esm_embeddings_path",
    type=str,
    default="~/PDBBIND/esm_embedding/esm_embedding_pocket_for_train/esm2_3billion_embeddings.pt",
    help="test dataset esmbedding path",
)
parser.add_argument(
    "--out_dir",
    type=str,
    default="~/test_workdir/mdn_result_40",
...
parser.add_argument(
    ...
    type=int,
    default=40,
    help="Number of poses to sample in parallel we recommand set number = batch_size_molecule*samples_per_complex",
)
parser.add_argument(
    "--batch_size_molecule",
    type=int,
    default=1,
    help="Number of molecul to sample in parallel",
)
parser.add_argument(
    "--cache_path",
    type=str,
    default="~/PDBBIND/cache_PDBBIND_pocket_8A",
    help="Folder from where to load/restore cached dataset",
)
parser.add_argument(
    "--data_dir",
    type=str,
    default="~/PDBBIND/PDBBind_pocket_8A/",
...
parser.add_argument(
    "--wandb_dir",
    type=str,
    default="~/test_workdir",
    help="Folder in which to save wandb logs",
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=int,
    default=20,
    help="Number of parallel workers for minimized.",
)
parser.add_argument(
...
parser.add_argument(
    "--save_docking_result_number",
    type=int,
    default=1,
    help="Number of poses to save in disk for each complex",
)
...
parser.add_argument(
    ...
    default="Screen",
    help="inference mode",
    choices=["Screen", "evaluate"],
)
parser.add_argument(
    "--head_index",
    type=int,
    default=0,
    help="the head index to start inference,this optinal to inference use multi-GPU every GPU minimized a part of csv file ",
)
parser.add_argument(
    "--tail_index",
    type=int,
    default=-1,
    help="the tail index to start inference,this optinal to inference use multi-GPU every GPU minimized a part of csv file",
)
parser.add_argument(
    "--ligandsMaxAtoms", type=int, default=80, help="the max number of atoms in ligand"
)
parser.add_argument("--random_seed", type=int, default=42, help="random seed")
# force_minimized param
parser.add_argument("--force_optimize", action="store_true", default=False, help="")
parser.add_argument(
    "--mdn_dist_threshold_test", type=float, default=3.0, help="mdn_dist_threshold_test"
)
...
def main_function():
    ...
    if args.confidence_model_dir is not None:
        with open(f"{args.confidence_model_dir}/model_parameters.yml") as f:
            ...
            confidence_args.mdn_dist_threshold_test = (
                args.mdn_dist_threshold_test
                if args.mdn_dist_threshold_test is not None
                else 5.0
            )
    ...
    if not args.no_model:
        model = get_model(
            ...
            no_parallel=True,
            model_type=score_model_args.model_type,
        )
    ...
    for (
        ...
        try:
            ...
            dirname = (
                os.path.splitext(pocket_path.split("/")[-1])[0]
                + "_"
                + os.path.splitext(ligands_path.split("/")[-1])[0]
            )
            write_dir = os.path.join(
                args.out_dir, "SurfDock_docking_result", dirname
            )  # f'{args.out_dir}/SurfDock_docking_result/{dirname}'
            os.makedirs(write_dir, exist_ok=True)

            esm_embeddings = copy.deepcopy(
                esm_embeddings_dict[os.path.splitext(os.path.basename(pocket_path))[0]]
            )

            test_dataset = ScreenDataset(
                pocket_path,
                ligands_path,
                ref_ligand,
                args.pocket_asl,
            ...
            test_loader = DataLoader(
                dataset=test_dataset, batch_size=args.batch_size_molecule, shuffle=False
            )
            if test_sample_num == 0:
                logger.error(
                    "No complexes need to be docking (skip before done or some errors) in {}",
                    pocket_path,
                )
                continue
            # test_loader= accelerator.prepare(test_loader)
            logger.info(
                "Protein {} Size of test dataset: {}",
                os.path.splitext(os.path.basename(pocket_path))[0],
                test_sample_num,
            )
            ...
            for idx, orig_complex_graph in tqdm(
                ...
                try:
                    if "ligand" not in orig_complex_graph.node_types:
                        logger.error(
                            "some error failed for conformer generate in rdkit: idx in batch graph: {}, ligand_path: {}",
                            idx,
                            ligands_path,
                        )
                        continue
                    ...
                    while (
                        not success
                    ):  # keep trying in case of failure (sometimes stochastic)
                        ...
                        try:
                            ...
                            randomize_position(
                                ...
                                ligand_to_pocket_center=args.ligand_to_pocket_center,
                            )
                            pdb = None
                            if args.save_visualisation:
                                ...
                                for idx, graph in enumerate(data_list):
                                    ...
                                    pdb.add(
                                        (
                                            orig_complex_graph_list[orig_complex_count][
                                                "ligand"
                                            ].pos
                                            + orig_complex_graph_list[
                                                orig_complex_count
                                            ].original_center
                                        )
                                        .detach()
                                    ...
                                    pdb.add(
                                        (
                                            graph["ligand"].pos
                                            + (graph.original_center).detach().cpu()
                                        ),
                                        part=1,
                                        order=1,
                                    )
                            ...
                            if not args.no_model:
                                ...
                                for _ in range(len(orig_complex_graph_list)):
                                    ...
                                    confidence_names.extend(
                                        [orig_complex_graph_list[_]["name"]] * N
                                    )
                                    pocket_path_list.extend(
                                        [os.path.basename(pocket_path)] * N
                                    )

                                sdf_names += [os.path.basename(ligands_path)] * len(
                                    confidence
                                )

                                assert (
                                    len(confidence_list)
                                    == len(confidence_names)
                                    == len(sdf_names)
                                    == len(pocket_path_list)
                                )
                            ...
                            if args.save_docking_result:
                                ...
                                for _ in range(len(orig_complex_graph_list)):
                                    for rank, batch_idx in enumerate(
                                        re_order[: args.save_docking_result_number]
                                    ):
                                        true_idx = head_threshold + batch_idx
                                        mol_pred = copy.deepcopy(
                                            data_list[true_idx]["mol"]
                                        )

                                        pos = (
                                            data_list[true_idx]["ligand"]
                                            .pos.cpu()
                                            .numpy()
                                            + orig_complex_graph_list[_]
                                            .original_center.cpu()
                                            .numpy()
                                        )
                                        ...
                                        if (args.inference_mode == "evaluate") and (
                                            ...
                                            try:
                                                ...
                                                logger.warning(
                                                    "Using non corrected RMSD because of the error:{}",
                                                    e,
                                                )
                                        ...
                                        write_mol_with_coords(
                                            mol_pred,
                                            pos,
                                            os.path.join(write_dir, result_filename),
                                        )
                            ...
                            if sample_count_failed > 5:
                                logger.error(
                                    " Skip by five times Failed on :{}, error of :{}",
                                    orig_complex_graph["name"],
                                    e,
                                )
                                success = 1
                    ...
                    orig_complex_graph_list, orig_complex_graph, data_list = (
                        None,
                        None,
                        None,
                    )
                    ...
                    torch.cuda.empty_cache()
                    logger.error(
                        "Some error failed for sampling: idx in batch : {}, ligand_path: {},error of :{} ",
                        idx,
                        ligands_path,
                        e,
                    )
            ...
            logger.error(
                "Some error failed for graph data. ligand_path: {},error of :{}",
                ligands_path,
                e,
            )
            (
        ...
        logger.info(
            "Protein {} used time: {}",
            os.path.splitext(os.path.basename(pocket_path))[0],
            time.time() - in_loop_start_time,
        )
    accelerator.wait_for_everyone()
    docking_time = time.time() - start_time
    if accelerator.is_local_main_process:
        logger.info(
            "Docking time used for one moleculer: {}", docking_time / all_molecules
        )
        logger.info("Docking time used: {}", docking_time)
        logger.info(
            "Sampling conformers number: {}", all_molecules * args.samples_per_complex
        )
        logger.info(
            "Output conformers number: {}",
            all_molecules * args.save_docking_result_number,
        )
    ...
    result = pd.DataFrame(
        {
            ...
            "confidence_name": confidence_names,
            "pocket_path": pocket_path_list,
        }
    )
    csv_flag = os.path.basename(args.data_csv).split(".")[0]
    result.to_csv(
        f"{args.out_dir}/{csv_flag}_head_{str(args.head_index)}_tail_{str(args.tail_index)}_confidence_on_device_{device}.csv",
        index=False,
    )
