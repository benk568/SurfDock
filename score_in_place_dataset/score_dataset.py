...
class ScreenDataset(Dataset):
    def __init__(
        ...
        # looking af the reusult dir to find the saved conformers and skip the ligand
        if not self.keep_input_pose:
            ...
            if len(finished_samples) == 0:
                ...
                max_inner_idx = [
                    int(re.findall(r"file_inner_idx_(\d+)", sample)[0])
    ...
    def get(self, idx):
        ...
        # if want to use SurfDock to generate conformers, use the following code
        """this module will work when you want to use SurfDock to generate conformers"""
        ...
        complex_graph["ligand", "lig_bond", "ligand"].edge_index = lig_complex_graph[
            "ligand", "lig_bond", "ligand"
        ].edge_index
        complex_graph["ligand", "lig_bond", "ligand"].edge_attr = lig_complex_graph[
            "ligand", "lig_bond", "ligand"
        ].edge_attr
        ...
        complex_graph.original_ligand_center = (
            ligand_center + complex_graph.original_center
        )
        # if self.pocket_center is not None:
        #     complex_graph['receptor'].pocket_center -= complex_graph.original_center.numpy()
    ...
    def get_complex(self, name, lm_embedding_chains):
        try:
            ...
            # pure_pocket_path = os.path.join(self.pdbbind_dir, name, f'{name}_pocket_pure.pdb')
        ...
        if os.path.isdir(self.ligands_path):
            ligands_paths = [
                path for path in os.listdir(self.ligands_path) if path.endswith(".sdf")
            ]
            num_cores = multiprocessing.cpu_count()
            ligs_paths = Parallel(
                n_jobs=min(len(ligands_paths), num_cores - 10), backend="threading"
            )(
                delayed(read_mol)(
            ...
            if not self.keep_input_pose:
                ...
                if len(ligs) > 0:
                    ...
                    ligand_names = [
                        ligand_name
                        for ligand_name, mol in zip(ligand_names, ligs)
                        if mol is not None and mol.GetNumConformers() > 0
                    ][self.finished_idx :]
            ...
            if self.inference_mode == "Screen":
                ...
                ligs = [
                    mol for mol in ligs if len(mol.GetAtoms()) <= self.ligandsMaxAtoms
                ]
            else:
        ...
        try:
            ...
            if (
                lm_embeddings is not None
                and c_alpha_coords is not None
        ...
        if self.surface_path is not None:
            ...
            with open(self.surface_path, "rb") as f:
                data = PlyData.read(f)
            features = [
                torch.tensor(data["vertex"][axis.name])
                for axis in data["vertex"].properties
                if axis.name not in ["nx", "ny", "nz"]
            ]
            pos = torch.stack(features[:3], dim=-1)
            # pos - center_protein_pos
            ...
            complex_graph["surface", "surface_edge", "surface"].edge_index = (
                data.edge_index
            )
            complex_graph["surface", "surface_edge", "surface"].edge_attr = (
                data.edge_attr
            )

            # add pocket center for later use
            if self.pocket_center is not None:
                complex_graph["receptor"].pocket_center = (
                    torch.tensor(self.pocket_center).float()
                    - complex_graph.original_center
                )
            else:
...
def read_mol(ref_path, remove_hs=False):
    ...
    if lig is None:  # read mol2 file if sdf file cannot be sanitized
        ...
        try:
            lig = read_molecule(
                os.path.splitext(ref_path)[0] + ".mol2",
                remove_hs=remove_hs,
                sanitize=True,
            )
        except:
...
def read_mols(ligands_path, sanitize=True, calc_charges=False, remove_hs=False):
    # def read_molecules(molecule_file, sanitize=False, calc_charges=False, remove_hs=False):
