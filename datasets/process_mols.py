        ...
        atom_features_list.append(
            [
                ...
                (
                    ...
                    if str(atom.GetChiralTag())
                    in allowable_features["possible_chirality_list"]
                    else 0
                ),
                safe_index(
    ...
    for residue_mdn in mdn_rec.residues:
        feature_list.append(
            [
                safe_index(
                    allowable_features["possible_amino_acids"],
                    obtain_resname(residue_mdn),
                )
            ]
            + obtain_self_dist(residue_mdn)
            + obtain_dihediral_angles(residue_mdn)
        )
...
def extract_receptor_structure(rec, lig, save_file, lm_embedding_chains=None):
    ...
    for i, chain in enumerate(rec):
        ...
        for res_idx, residue in enumerate(chain):
            ...
            for atom in residue:
                # Keep track specifically of backbone atom coords, as well as all atoms
                if atom.name == "CA":
    ...
    for i, chain in enumerate(rec):
        if chain.get_id() in valid_chain_ids:
            ...
            # Get all chains that haven't been decided to be valid
    ...
    # Remove all invalid chains from the receptor structure
    ...
    if lm_embedding_chains is not None:
        logger.info(
            f"Found {len(lm_embedding_chains)} LM embeddings for {len(c_alpha_coords)} residues"
        )
        assert len(lm_embedding_chains) == len(n_coords)
    # save_file (which contains rec) seems to be the main output of this function, even
    #  though it does return stuff if the file doesn't already exist
    io = PDBIO()
...
def get_lig_graph(mol, complex_graph, use_chirality=True):
    ...
    for bond in mol.GetBonds():
        ...
        bt = bond.GetBondType()
        bond_feats = [
            bt == BT.SINGLE or bt == BT.UNSPECIFIED,
            bt == BT.DOUBLE,
            bt == BT.TRIPLE,
            bt == BT.AROMATIC,
...
def initConformer(lig_mol, inference_mode="Screen"):
    ...
    generate_conformer(
        lig_mol, useRandomCoords=False if inference_mode == "Screen" else True
    )
...
def get_lig_graph_with_matching(
    ...
    if matching:
        ...
        if not rotable_bonds:
            logger.info("no_rotable_bonds but still using it")
...
def get_calpha_graph(
    ...
    edge_connect = torch.tensor(
        np.array([check_connect(mdn_rec, x, y) for x, y in zip(src_list, dst_list)])
    )
    complex_graph["receptor", "rec_contact", "receptor"].edge_attr = torch.cat(
        [
    ...
    complex_graph["receptor"].atoms_pos = torch.tensor(
        np.array(
            [
                np.concatenate(
                    [
                        res.atoms.positions,
                        np.full(
                            (np.max([RES_MAX_NATOMS - len(res.atoms), 0]), 3), np.nan
                        ),
                    ],
                    axis=0,
                )[:RES_MAX_NATOMS]
                for res in mdn_rec.residues
            ]
        )
    )
