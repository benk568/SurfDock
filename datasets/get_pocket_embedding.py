...
parser.add_argument(
    "--protein_pocket_csv",
    type=str,
    default="~/processsed/PDBBIND.csv",
    help="save pocket and full protein csv locations",
)
parser.add_argument(
    "--embeddings_dir",
    type=str,
    default="~/esm_embedding/esm_embedding_output",
    help="full protein embedding dir locations",
)
parser.add_argument(
    "--pocket_emb_save_dir",
    type=str,
    default="~/esm_embedding/esm_embedding_output_pocket_new",
    help="",
)
...
for pbar_idx, (full_protein_path, pocket_path) in enumerate(pbar):
    ...
    try:
        ...
        full_structure = biopython_parser.get_structure(
            f"{protein_name}", full_protein
        )[0]
        pocket_embeddings = []
        pocket_infos_all = []
        for i, chain in enumerate(full_structure.get_chains()):
            ...
            try:
                ...
                pbar.set_description(
                    f"{chain_id} not in {protein_name} pocket skip this chain"
                )
                continue
            try:

                embeddings_path_chain = os.path.join(
                    embeddings_dir,
                    f"{os.path.basename(full_protein_path)}_chain_{i}.pt",
                )
                ...
                assert len(list(chain.get_residues())) == len(
                    embeddings
                ), "embedding must equal to res nums!"
                ...
                assert len(list(chain.get_residues())) == len(
                    embeddings
                ), f"embedding must equal to res nums! {len(list(chain.get_residues()))},{len(embeddings)}"
            pocket_infos = []
        ...
        assert len(pocket_embeddings) == len(
            pocket_infos_all
        ), f"pocket embedding must equal to res nums! {len(pocket_embeddings)},{len(pocket_infos_all)}"
        torch.save(
            pocket_embeddings, os.path.join(pocket_emb_save_dir, f"{protein_name}.pt")
        )
